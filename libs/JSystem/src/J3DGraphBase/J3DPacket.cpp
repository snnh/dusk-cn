...
void J3DMatPacket::draw() {
...
#if DUSK_TPHD
    {
        ...
        if (offs != NULL) {
            GX2SetPolygonOffset(
                offs->mFrontOffset, offs->mFrontScale, offs->mBackOffset, offs->mBackScale,
                offs->mClamp);
        } else {
            // Non-Full PE blocks have no stored polygon offset. Reset the state left by the
            // preceding material instead of dereferencing their null default.
